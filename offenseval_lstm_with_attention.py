# -*- coding: utf-8 -*-
"""offenseval_lstm_with_attention.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1-yaOT8v2SySVzgQpy3BrIidZuw9lFbcb
"""
...
def load_train():
  train = pd.read_csv(io.StringIO(uploaded['cleaned_set.tsv'].decode('utf-8')), sep='\t')
  train = train.drop(columns = ['subtask_b', 'subtask_c'])
  train.replace(np.nan, "nothing", inplace=True)
  train = train.values
  corpus = train[:,4]
  y = train[:,3]
...
def train_epoch(model, train_data, y, loss_function, optimizer, i):
    ...
    print('epoch: %d done! \n train avg_loss:%g , acc:%g'%(i, avg_loss, f1_score(truth_res, pred_res, average='macro')))
...
#     acc = get_accuracy(truth_res, pred_res)
    f1score = f1_score(truth_res, pred_res, average='macro')
    print(name + ' avg_loss:%g train f1:%g' % (avg_loss, f1score ))
    return f1score


def save_clean_data(lemmatized_data):
	train = pd.read_csv('data/offenseval-training-v1.tsv', sep='\t')
	train['lemmatized_data'] = pd.Series(lemmatized_data, name = "cleaned_data")
	train.to_csv("data/cleaned_set.tsv", sep= "\t")

if __name__ == "__main__":
	lemmatized_data, y = load_train()
	# lemmatized_data = clean_data(corpus)
	# save_clean_data(lemmatized_data)
	print(y)
	# del corpus
	all_words = set()
	...
	model = LSTMClassifier(embedding_dim=100,hidden_dim=100,
						vocab_size=len(all_words),label_size=2, atten_mod=attention, dropout_p=0.4)
  ...
  # review this part 
	...
	for i in range(EPOCH):
		...
		print('now best dev acc:',best_dev_f1)
		dev_f1 = evaluate(model,test_corp, test_y,loss_function,'dev')
		if dev_f1 > best_dev_f1:
			best_dev_f1 = dev_f1
# 			os.system('rm mr_best_model_acc_*.model')
			print('New Best Dev!!!', dev_f1)
			torch.save(model.state_dict(), 'mr_best_model_acc_' + str(int(dev_f1*10000)) + '.model')
# 			files.download('mr_best_model_acc_' + str(int(dev_acc*10000)) + '.model')
			no_up = 0
		else:
			no_up += 1
			if no_up >= 3:
...
!ls
files.download('mr_best_model_acc_7170.model')
